...
已提供专用公开演示入口 `demo_app.py`，可部署至Streamlit Community Cloud；步骤见 [DEPLOY.md](DEPLOY.md)。每位访客使用独立的临时内存SQLite，只处理内置模拟资料，不开放上传或模型调用；仍可完成提取、对比、价值说明、问答、审核和导出。`app.py` 保留原本的本地完整功能。没有在此预填未经验证的线上网址。
